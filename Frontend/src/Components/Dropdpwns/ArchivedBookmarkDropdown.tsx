...
const ArchivedBookmarkDropdown = ({ id, url, isOpen, copy, visit }: DropdownProps) => {
	...
	return (
		<div className={`absolute ${isOpen ? "grid" : "hidden"} w-54 right-0 top-[calc(100%+0.5rem)] gap-2 p-2 bg-neutral-light-0 dark:bg-neutral-dark-600 rounded-lg border border-neutral-light-100 dark:border-neutral-dark-500 shadow-[0px_6px_14px_0px,rgba(34,38,39,0.12)]`}>
			...
			<button type="button" onClick={() => open(`unarchive-dialog-${id}`)} className="p-2 flex items-center gap-2.5 cursor-pointer">
			...
			<button type="button" onClick={() => open(`delete-dialog-${id}`)} className="p-2 flex items-center gap-2.5 text-start cursor-pointer">
