import { SquareArrowOutUpRight, Copy, Archive, Pin, SquarePen } from "lucide-react";
...
const ActiveBookmarkDropdown = ({ id, url, isOpen, copy, visit, pin }: DropdownProps) => {
	const { open } = useUI();
	...
	return (
		<div className={`grid absolute ${isOpen ? "pointer-events-auto opacity-100 transition-opacity duration-200 ease-in" : "pointer-events-none opacity-0 transition-opacity duration-200 ease-in"} w-48 right-0 top-[calc(100%+0.5rem)] gap-2 p-2 bg-neutral-light-0 dark:bg-neutral-dark-600 rounded-lg border border-neutral-light-100 dark:border-neutral-dark-500 shadow-[0px_6px_14px_0px_rgba(34,38,39,0.12)]`}>
			<a onClick={visit} target="_blank" href={url} className="p-2 flex items-center gap-2.5 text-sm font-semibold text-neutral-light-800 dark:text-neutral-dark-100  hover:bg-neutral-light-100 dark:hover:bg-neutral-dark-500 rounded-sm">
				<SquareArrowOutUpRight className="text-neutral-light-800 dark:text-neutral-dark-100" />
				Visit
			</a>
			<button onClick={copy} className="p-2 flex items-center gap-2.5 cursor-pointer hover:bg-neutral-light-100 dark:hover:bg-neutral-dark-500 rounded-sm">
			...
			<button onClick={pin} type="button" className="p-2 flex items-center gap-2.5 cursor-pointer hover:bg-neutral-light-100 dark:hover:bg-neutral-dark-500 rounded-sm">
				<Pin className="text-neutral-light-800 dark:text-neutral-dark-100" />
				<p className="text-sm font-semibold text-neutral-light-800 dark:text-neutral-dark-100">Pin</p>
			</button>
...
export default ActiveBookmarkDropdown;